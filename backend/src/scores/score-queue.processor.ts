...
export class ScoreQueueProcessor implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  private async processOne(): Promise<void> {
    ...
    try {
      ...
      let scoreDetails: { item_name: string; score: number; comment: string }[] = [];
      let totalScore = 0;
      try {
        const result = await this.scoresService.scoreWorkContentWithLlm(criteriaMarkdown, work.content, {
          timeoutMs: LLM_TIMEOUT_MS,
          context: { departmentName: work.department_name, positionName: work.position_name },
        });
      ...
      if (existingAi) {
        db.prepare('UPDATE score_queue SET status = ?, processed_at = ?, error_message = NULL WHERE id = ?').run('done', now, row.id);
        return;
      }
