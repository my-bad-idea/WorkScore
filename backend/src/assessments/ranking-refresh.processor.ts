...
export class RankingRefreshProcessor implements OnApplicationBootstrap, OnModuleDestroy {
  ...
  private async computeWorkPlanScore(
    ...
    const prompt = `你是一位工作计划完成度评分员。根据以下某用户在某月的工作计划及执行情况，给出一个 0–100 的完成度总分。

## 评分规则
- 考虑：计划是否具体、状态（如已完成/进行中/未开始）、是否按计划或实际时间完成。
- 优秀 85-100：计划清晰且多数已完成或按计划推进；良好 70-84：大部分完成或有合理说明；一般 55-69：部分完成；不足 0-54：几乎未执行或计划空洞。
- 仅返回一个 JSON 对象，不要任何其他说明。格式：{"score": 数字}

## 参考示例
计划："完成模块A开发" 状态: 已完成 计划: 2024-01-01~01-07 实际: 2024-01-01~01-05 → {"score": 92}
计划："调研方案" 状态: 未开始 无时间 → {"score": 15}
